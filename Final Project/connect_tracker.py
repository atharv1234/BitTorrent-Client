...
class udp_torrent_tracker(tracker_data):
    ...
    def parse_connection_response(self, raw_connection_data):
        ...
        if(len(raw_connection_data) < 16):
            raise RuntimeError(' Wrong reponse length of connection ID UDP tracker!')
        ...
        if(response_transaction_id != self.transaction_id):
            raise RuntimeError(' Wrong response transaction ID of UDP tracker !')
    ...
    def parse_udp_tracker_response(self, raw_announce_reponse):
        if(len(raw_announce_reponse) < 20):
            raise RuntimeError('Incorrect response length in announcing!')
        ...
        if response_transaction_id != self.transaction_id:
            raise RuntimeError('The transaction id in annouce response doescd  not match')
        
        # check if the response contains any error message
        if response_action != 0x1:
            error_msg = struct.unpack_from("!s", raw_announce_reponse, 8)
            raise RuntimeError("Error while announcing: %s" % error_msg)
...
class torrent():
    ...
    def get_piece_length(self, piece_index):
        ...
        if piece_index == self.pieces_count - 1:
            return self.torrent_metadata.fileSize - self.torrent_metadata.pieceLength * (piece_index)
        else:
            return self.torrent_metadata.pieceLength
